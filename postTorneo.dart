...
class DataJugadorScoreStb100 {
  ...
  factory DataJugadorScoreStb100.fromJson2(Map<String, dynamic> json) {
    return DataJugadorScoreStb100.origen(
        firmaMarcadorMatricula: json['firma_marcador_matricula'].toString(),
        firmaMarcadorImage:
        UserFunctions.toUintImage(json['firma_marcador_image'] ?? ''),
        firmaUserImage: UserFunctions.toUintImage(json['firma_user_image'] ?? ''),
  ...
  addHoyo(int hoyoNro, int distancia, int par, int handicap) {
    ...
    /// STABLEFORD ----------------------------------------------
    int hcpStbTorneo = 0;
    int hcpStb100Torneo=(hcpTorneo).round(); ///100%
    print("hcpStbTorneo---------------------");
    print(hcpStb100Torneo);
    // int hcpStbTorneo=(hcpTorneo*1).round();
    if (hcpStb100Torneo < 0) {
      hcpStb100Torneo = (hcpStb100Torneo ~/ 18) +
          UserFunctions.miif((hcpStb100Torneo % 18) < handicap, -1, 0);
    } else {
      hcpStb100Torneo = (hcpStb100Torneo ~/ 18) +
          UserFunctions.miif((hcpStb100Torneo % 18) >= handicap, 1, 0);
    }
    ...
    hoyos[hoyoNro - 1] = new DataHoyoJuego(
        hoyoNro, distancia, par, handicap, golpesHcp, 0, 0, 0,hcpStb100Torneo);
  }
